...
- **Entry points** (`cmd/`): `server` (HTTP API), `agentharness` (CLI — daemon, agent management, config), `migrate`
...
- **CLI** (`internal/cli/`): Shared helpers for the `agentharness` CLI — API client, config management, output formatting.
...
pnpm --filter @agentharness/web exec vitest run src/path/to/file.test.ts
