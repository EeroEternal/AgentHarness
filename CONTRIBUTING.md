...
This guide documents the local development workflow for contributors working on the AgentHarness codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=agentharness`
...
```bash
POSTGRES_DB=agentharness_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://agentharness:agentharness@localhost:5432/agentharness_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../agentharness-feature -b feat/my-change main
cd ../agentharness-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `agentharness_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`agentharness login`).
...
```bash
docker compose exec -T postgres psql -U agentharness -d postgres -At -c "select datname from pg_database order by datname;"
```
