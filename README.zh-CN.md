<p align="center">
  <img src="docs/assets/banner.jpg" alt="Agent Harness — 人类与 AI，并肩前行" width="100%">
</p>
...
<picture>
  ...
  <img alt="Agent Harness" src="docs/assets/logo-light.svg" width="50">
</picture>
...
[官网](https://openagent.run) · [云服务](https://openagent.run) · [X](https://x.com/multica_hq) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)
...
Agent Harness 将编码 Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、报告阻塞问题、更新状态。
...
<p align="center">
  <img src="docs/assets/landing-hero-light.png" alt="Agent Harness 看板视图" width="800">
</p>

## 功能特性

Agent Harness 管理完整的 Agent 生命周期：从任务分配到执行监控再到技能复用。
...
最快的上手方式，无需任何配置：**[openagent.run](https://openagent.run)**
...
`AgentHarness` CLI 将你的本地机器连接到 Multica — 用于认证、管理工作区和运行 Agent daemon。
...
安装好 CLI（或注册 [AgentHarness 云服务](https://openagent.run)）后，按以下步骤将第一个任务分配给 Agent：

### 1. 登录并启动 daemon

```bash
multica login           # 使用你的 AgentHarness 账号认证
