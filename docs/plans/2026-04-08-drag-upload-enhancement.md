...
**File Card Node** — a Tiptap custom node that renders `[name](url)` as a styled card when the URL matches our CDN (`agentharness-static.copilothub.ai` or S3 bucket domain).
...
Markdown storage:   [report.pdf](https://agentharness-static.copilothub.ai/xxx.pdf)
```
