...
export const useWorkspaceClientStore = create<WorkspaceClientState>((set) => ({
  currentWorkspaceId: localStorage.getItem("agentharness_workspace_id"),
  setCurrentWorkspaceId: (id) => {
    if (id) {
      localStorage.setItem("agentharness_workspace_id", id);
      api.setWorkspaceId(id);
    } else {
      localStorage.removeItem("agentharness_workspace_id");
...
4. **Import alias `@core/*`** — In Phase 6, change tsconfig to point `@agentharness/core/*` to `packages/core/`, or use package.json workspace imports.
