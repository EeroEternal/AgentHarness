...
**Architecture:** `@agentharness/core` 自带完整初始化（API、stores、WS），不需要每个 app 调用 factory。`@agentharness/views` 包含所有页面和 layout。每个 app 只提供路由壳子。
...
方案：`@agentharness/core` 导出一个 `<CoreProvider>` 包裹整个应用。它内部自动完成所有初始化。配置通过环境变量（`VITE_API_URL` / `NEXT_PUBLIC_API_URL`）或 prop 注入。SSR-safe 的 localStorage wrapper 内置到 core 里作为默认 storage（`typeof window` 守卫对 Electron 无害）。
...
### Task 1: 在 `@agentharness/core` 里创建 CoreProvider
...
export function AuthInitializer({
  ...
  useEffect(() => {
    const token = defaultStorage.getItem("agentharness_token");
    if (!token) {
      onLogout?.();
    ...
    const api = getApi();
    api.setToken(token);
    const wsId = defaultStorage.getItem("agentharness_workspace_id");
    ...
    Promise.all([api.getMe(), api.listWorkspaces()])
      ...
      .catch((err) => {
        ...
        defaultStorage.removeItem("agentharness_token");
        defaultStorage.removeItem("agentharness_workspace_id");
        onLogout?.();
...
- `from "@/platform/api"` → `from "@agentharness/core/api"` (use singleton proxy `api`)
- `from "@/platform/auth"` → `from "@agentharness/core/auth"` (use singleton `useAuthStore`)
- `from "@/platform/workspace"` → `from "@agentharness/core/workspace"` (use singleton `useWorkspaceStore`)
...
### Task 3: Extract `AppSidebar` to `@agentharness/views/layout`
...
- `import { useAuthStore } from "@agentharness/core/auth"` (singleton)
- `import { useWorkspaceStore } from "@agentharness/core/workspace"` (singleton)
- `import { api } from "@agentharness/core/api"` (singleton proxy)
- `import { useNavigation, AppLink } from "../navigation"` (relative within views)
- `import { useModalStore } from "@agentharness/core/modals"`
- All `@agentharness/ui` imports unchanged
...
```bash
git commit -m "refactor(views): extract shared AppSidebar to @agentharness/views/layout"
```

---

### Task 4: Extract `DashboardLayout` to `@agentharness/views/layout`
...
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <DashboardLayout
      loadingIndicator={<AgentHarnessIcon className="size-6" />}
      extra={<SearchCommand />}
    >
      {children}
    </DashboardLayout>
...
```bash
git commit -m "refactor(views): extract shared DashboardLayout to @agentharness/views/layout"
```
...
### Task 6: Extract `LoginPage` to `@agentharness/views/auth`
...
```bash
git commit -m "feat(views): extract shared LoginPage to @agentharness/views/auth"
```
...
### Task 8: Create `@agentharness/views/agents`
...
**Key migration:** All `@/platform/*` imports → `@agentharness/core/*` singletons. All `@agentharness/ui` and `@agentharness/core` imports stay as-is. `@agentharness/views` imports become relative.
...
```bash
git commit -m "feat(views): extract agents page to @agentharness/views/agents"
```
...
export { AgentsPage as default } from "@agentharness/views/agents";
```

Commit: `refactor(web): replace agents page with @agentharness/views/agents import`
...
Commit: `feat(desktop): wire agents page from @agentharness/views`

---

## Phase 5: Extract Inbox Page (468 lines → shared module)

### Task 11: Create `@agentharness/views/inbox`
...
- `@/platform/*` → `@agentharness/core/*` singletons

Commit: `feat(views): extract inbox page to @agentharness/views/inbox`
...
export { InboxPage as default } from "@agentharness/views/inbox";
```

Commit: `refactor(web): replace inbox page with @agentharness/views/inbox import`
...
Commit: `feat(desktop): wire inbox page from @agentharness/views`

---

## Phase 6: Extract Settings Page (1,277 lines → shared module)

### Task 14: Create `@agentharness/views/settings`
...
**Key migration:** Same pattern — `@/platform/*` → `@agentharness/core/*` singletons.

Commit: `feat(views): extract settings page to @agentharness/views/settings`
...
export { SettingsPage as default } from "@agentharness/views/settings";
```

Delete `apps/web/app/(dashboard)/settings/_components/` (all 6 files).

Commit: `refactor(web): replace settings page with @agentharness/views/settings import`
...
Commit: `feat(desktop): wire settings page from @agentharness/views`
